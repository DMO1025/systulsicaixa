...
import type { IndividualPeriodConfig as PeriodConfig, IndividualSubTabConfig as SubTabConfig, SalesChannelId, GroupedChannelConfig } from '@/lib/config/forms';
...
const AlmocoPrimeiroTurnoForm: React.FC<PeriodFormProps> = ({
  ...
  const periodTotal = useMemo(() => {
    ...
    return totals.turnos.almocoPT.valor;
  }, [watchedData]);
      ...
      <CardHeader>
        <div className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between gap-2">
          ...
          <div className="text-left sm:text-right">
            ...
            <p className="text-xs text-muted-foreground mt-1">(Almoço 1º Turno + Frigobar + Reajuste C.I.)</p>
          </div>
        </div>
                ...
                <TabsContent key={subTabKey} value={subTabKey}>
                  <Card className={cn("border-2 transition-all", activeSubTabs[periodId] === subTabKey ? 'border-primary/20 ring-1 ring-primary/10' : 'border-border')}>
                    ...
                    <CardContent className="p-4">
                      {subTabKey === 'faturado' ? (
                        <FaturadoForm form={form} basePath="almocoPrimeiroTurno.subTabs.faturado" />
                      ) : subTabKey === 'consumoInterno' ? (
                        <ConsumoInternoForm form={form} basePath="almocoPrimeiroTurno.subTabs.consumoInterno" />
                      ) : (
                        renderChannelInputs(
