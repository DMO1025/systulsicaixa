...
import { processEntryForTotals } from '@/lib/reports/daily/generator';
...
const SingleDayReportView: React.FC<SingleDayReportViewProps> = ({ entry }) => {
    ...
    const totals = useMemo(() => {
        const processed = processEntryForTotals(entry).totals;
    ...
    return (
        <div className="space-y-6">
            ...
            {PERIOD_DEFINITIONS.map(pDef => {
                ...
                const periodTotals = processEntryForTotals(entry).totals;
                
                let periodTotal = 0;
                let periodQtd = 0;
                
                if (pDef.id === 'madrugada') {
                    periodTotal = periodTotals.rsMadrugada.valor;
                    periodQtd = periodTotals.rsMadrugada.qtdPedidos;
                } else if (pDef.id === 'almocoPrimeiroTurno') {
                    periodTotal = periodTotals.turnos.almocoPT.valor;
                    periodQtd = periodTotals.turnos.almocoPT.qtd;
                } else if (pDef.id === 'almocoSegundoTurno') {
                    periodTotal = periodTotals.turnos.almocoST.valor;
                    periodQtd = periodTotals.turnos.almocoST.qtd;
                } else if (pDef.id === 'jantar') {
                    periodTotal = periodTotals.turnos.jantar.valor;
                    periodQtd = periodTotals.turnos.jantar.qtd;
                } else if (pDef.id === 'cafeDaManha') {
                    periodTotal = periodTotals.cafeHospedes.valor + periodTotals.cafeAvulsos.valor;
                    periodQtd = periodTotals.cafeHospedes.qtd + periodTotals.cafeAvulsos.qtd;
                } else {
                    const genericTotal = (periodTotals as any)[pDef.id];
                    if (genericTotal) {
                        periodTotal = genericTotal.valor || 0;
                        periodQtd = genericTotal.qtd || 0;
                    }
                }
