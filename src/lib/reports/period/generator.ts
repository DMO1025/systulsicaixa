...
export function generatePeriodReportData(entries: DailyLogEntry[], periodId: PeriodId | 'all' | 'consumoInterno' | 'faturado' | 'frigobar' | 'roomService'): PeriodReportViewData {
    ...
    if (periodId === 'roomService') {
        entries.forEach(entry => {
            ...
            const { rsMadrugada, rsAlmocoPT, rsAlmocoST, rsJantar } = totals;
            ...
            if (rsAlmocoPT.qtd || rsAlmocoPT.valor) {
                addToBreakdown('rsAlmocoPT', { date, qtd: rsAlmocoPT.qtd, valor: rsAlmocoPT.valor });
                addToSummary('rsAlmocoPT', rsAlmocoPT.qtd, rsAlmocoPT.valor);
            }
            if (rsAlmocoST.qtd || rsAlmocoST.valor) {
                addToBreakdown('rsAlmocoST', { date, qtd: rsAlmocoST.qtd, valor: rsAlmocoST.valor });
                addToSummary('rsAlmocoST', rsAlmocoST.qtd, rsAlmocoST.valor);
            }
            if (rsJantar.qtd || rsJantar.valor) {
                addToBreakdown('rsJantar', { date, qtd: rsJantar.qtd, valor: rsJantar.valor });
                addToSummary('rsJantar', rsJantar.qtd, rsJantar.valor);
            }
        });
        ...
        entries.forEach(entry => {
            const date = format(parseISO(String(entry.id)), 'dd/MM/yyyy');
            const eventosData = entry.eventos as PeriodData | undefined;
        ...
        entries.forEach(entry => {
            ...
            if(cdmListaQtd || cdmListaVlr) {
                addToBreakdown('cdmListaHospedes', {date, qtd: cdmListaQtd, valor: cdmListaVlr});
